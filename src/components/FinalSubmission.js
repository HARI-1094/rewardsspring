...
import "../styles/FinalSubmission.css";
...
const FinalSubmission = () => {
  ...
  const handlePidChange = async (event) => {
    ...
    if (pid) {
      try {
        const response = await axios.get(`http://localhost:8080/api/final_submissions/${pid}`);
  ...
  const handleSubmit = async () => {
    if (selectedPid && marks) {
      try {
        ...
        await axios.post("http://localhost:8080/api/final_submissions", payload);
  ...
  return (
    <div className="final-submission1">
